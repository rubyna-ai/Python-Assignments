...
def convert_date(date_str, from_cal, to_cal):
    ...
    # split "1985-06-24" into year=1985, month=6, day=24
...
def format_date(year, month, day, to_cal, style):
    ...
    # helper: turn 24 into "24th", 1 into "1st", 2 into "2nd", 3 into "3rd"
    def ordinal(d):
        if d in [1, 21, 31]:
            return f"{d}st"
        elif d in [2, 22]:
...
# loop through each customer and print result
for c in customers:
    # step 1: convert the date
    year, month, day = convert_date(c["date"], c["cal"], c["need"])

    # step 2: format the converted date based on style
